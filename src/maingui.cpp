...
)
{
	...
	for (auto entry : fs::directory_iterator(resource_path + "/maps/"))
...
void mapselgui_refresh(GUIPage &page)
{
	...
	mapcolumns(
		page.things, page.font, 5, page.things.back().r,
		page.gd->resource_path
	);
...
int mapselgui_click(GUIPage* page, GUIThing* thing)
{
	...
	{
		loadmap(
			page->gd->map, page->gd->pos, thing->value, page->gd->resource_path
		);
		return -1;
	}
	// refresh
...
)
{
	...
	mapcolumns(guithings, font, 3, ref, resource_path);
...
int editgui_click(GUIPage* page, GUIThing* thing)
{
	...
	case EGSAVE: // save map
		if (
			savemap(
				page->gd->map, page->things[EGNAME].value,
				page->gd->resource_path
			)
		) return 0;
		// update map listing
		...
		page->bdr = editgui_listmaps(
			page->things, page->font, page->gd->map->name,
			page->gd->resource_path
		);
		break;
	// load map
	case EGLOAD:
		loadmap(
			page->gd->map, page->gd->pos, page->things[EGNAME].value,
			page->gd->resource_path
		);
		break;
	// new map
...
void setup_editgui(SETUP_ARGS)
{
	...
	page.bdr = editgui_listmaps(
		page.things, font, gd->map->name, gd->resource_path
	);
	center_page(page, { draw_w / 2, draw_h / 2 });
}
